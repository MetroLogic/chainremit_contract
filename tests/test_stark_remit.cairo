...
use snforge_std::{
    ContractClassTrait, DeclareResultTrait, Event, EventSpyAssertionsTrait, declare, load,
    spy_events, start_cheat_caller_address_global, stop_cheat_caller_address_global,
};
...
use starkremit_contract::starkremit::StarkRemit::{
    AgentAuthorized, AgentPermissionUpdated, AuditTrailEntry, ContractUpgradeCompleted,
    ContractUpgradeRolledBack, EmergencyPauseActivated, EmergencyPauseDeactivated,
    MultiSigOperationApproved, MultiSigOperationExecuted, MultiSigOperationProposed,
};
...
fn deploy() -> (IStarkRemitDispatcher, ContractAddress) {
    ...
    let (contract_address, _) = contract
        .deploy(@array![owner.into(), oracle.into(), token.into()])
        .unwrap();
...
fn test_authorize_agent() {
        ...
        .assert_emitted(
            @array![
                (
                    ...
                    StarkRemit::Event::AgentAuthorized(
                        AgentAuthorized {
                            agent_address: agent,
                            permission: 'WRITE',
                            authorized: true,
    ...
    let is_authorized = load(
        contract.contract_address, selector!("agent_permissions"), 1 // Length of the storage slot
    );
    ...
    spy
        .assert_emitted(
            @array![
                (
                    contract.contract_address,
                    StarkRemit::Event::ContractUpgradeCompleted(
...
fn test_emergency_pause() {
    ...
    spy
        .assert_emitted(
            @array![
                (
                    contract.contract_address,
                    StarkRemit::Event::EmergencyPauseActivated(
                        EmergencyPauseActivated { function_selector, caller: admin, expires_at },
                    ),
                ),
            ],
        );
    ...
    // Verify pause status
    let is_paused = load(contract.contract_address, selector!("paused_functions"), 1);
    ...
    spy
        .assert_emitted(
            @array![
                (
                    contract.contract_address,
                    StarkRemit::Event::EmergencyPauseDeactivated(
                        EmergencyPauseDeactivated { function_selector, caller: admin },
                    ),
                ),
            ],
        );
    ...
    spy
        .assert_emitted(
            @array![
                (
                    contract.contract_address,
                    StarkRemit::Event::MultiSigOperationProposed(
                        MultiSigOperationProposed {
                            op_id, target_contract, selector, proposer: signer1,
                        },
                    ),
                ),
            ],
    ...
    spy
        .assert_emitted(
            @array![
                (
                    contract.contract_address,
                    StarkRemit::Event::MultiSigOperationApproved(
                        MultiSigOperationApproved {
                            op_id, approver: signer2, confirmations_count: 2,
                        },
                    ),
                ),
            ],
    ...
    spy
        .assert_emitted(
            @array![
                (
                    contract.contract_address,
                    StarkRemit::Event::MultiSigOperationExecuted(
                        MultiSigOperationExecuted { op_id, executor: signer2 },
                    ),
                ),
            ],
        );
...
fn test_audit_trail() {
    ...
    spy
        .assert_emitted(
            @array![
                (
                    contract.contract_address,
                    StarkRemit::Event::AuditTrailEntry(
                        AuditTrailEntry {
                            action, actor: owner, timestamp: get_block_timestamp(), details,
                        },
                    ),
                ),
            ],
    ...
    let audit_entry: AuditEntry = load(
        contract.contract_address, selector!("audit_trail"), 4 // Number of fields in AuditEntry
    )
        .try_into()
        .unwrap();
...
fn test_rollback_contract() {
    ...
    spy
        .assert_emitted(
            @array![
                (
                    contract.contract_address,
                    StarkRemit::Event::ContractUpgradeRolledBack(
    ...
    let current_version = load(contract.contract_address, selector!("upgrade_count"), 1);
...
fn test_revoke_permission() {
    ...
    spy
        .assert_emitted(
            @array![
                (
                    contract.contract_address,
                    StarkRemit::Event::AgentPermissionUpdated(
    ...
    // Verify storage update
    let is_authorized = load(contract.contract_address, selector!("agent_permissions"), 1);
