...
fn test_kyc_management() {
    let (_, starkremit_dispatcher, _) = __setup__();
    let user = contract_address_const::<'kyc_user'>();
    let admin = contract_address_const::<'admin'>();
    let registration_data = create_test_registration();

    // Grant ADMIN_ROLE for KYC management (OpenZeppelin AccessControl)
    start_cheat_caller_address_global(OWNER());
    starkremit_dispatcher.grant_admin_role(admin);
    stop_cheat_caller_address_global();
    ...
    let result = starkremit_dispatcher
        .update_kyc_status(
            ...
            KycLevel::Basic,
            'verification_hash',
            2000000000 // expires_at (future timestamp)
        );
    stop_cheat_caller_address_global();

    assert_eq!(result, true, "KYC update should succeed");

    // Check KYC status
    let (status, level) = starkremit_dispatcher.get_kyc_status(user);
    assert_eq!(status, KycStatus::Approved, "KYC status should be approved");
    assert_eq!(level, KycLevel::Basic, "KYC level should be basic");

    // Check if KYC is valid
    let is_valid = starkremit_dispatcher.is_kyc_valid(user);
    assert_eq!(is_valid, true, "KYC should be valid");
}
...
fn test_agent_registration() {
    ...
    // Grant ADMIN_ROLE for agent registration (OpenZeppelin AccessControl)
    start_cheat_caller_address_global(OWNER());
    starkremit_dispatcher.grant_admin_role(admin);
    stop_cheat_caller_address_global();
    ...
    let result = starkremit_dispatcher
        .register_agent(
            ...
            'US', // primary_region
            'CA', // secondary_region
            500 // commission_rate (5%)
        );
    stop_cheat_caller_address_global();

    assert_eq!(result, true, "Agent registration should succeed");

    // Get agent details
    let agent_data = starkremit_dispatcher.get_agent(agent);
    assert_eq!(agent_data.agent_address, agent, "Agent address should match");
    assert_eq!(agent_data.name, 'Agent Name', "Agent name should match");
    assert_eq!(agent_data.status, AgentStatus::Active, "Agent should be active");
    assert_eq!(agent_data.commission_rate, 500, "Commission rate should match");
}
...
fn test_transfer_creation() {
    ...
    // Register both users with unique emails
    let sender_data = create_unique_registration('sender@test.com');
    let recipient_data = create_unique_registration('recipient@test.com');

    start_cheat_caller_address_global(sender);
    starkremit_dispatcher.register_user(sender_data);
    stop_cheat_caller_address_global();

    start_cheat_caller_address_global(recipient);
    starkremit_dispatcher.register_user(recipient_data);
    stop_cheat_caller_address_global();

    // Create transfer
    start_cheat_caller_address_global(sender);
    let transfer_id = starkremit_dispatcher
        .create_transfer(
            recipient,
            1000, // amount
            2000000000, // expires_at (future timestamp)
            'test_transfer' // metadata
        );
    stop_cheat_caller_address_global();

    // Transfer should be created successfully, check the returned ID is valid
    // Don't assume ID is 0, just verify transfer was created

    // Get transfer details using the returned ID
    let transfer = starkremit_dispatcher.get_transfer(transfer_id);
    assert_eq!(transfer.sender, sender, "Sender should match");
    assert_eq!(transfer.recipient, recipient, "Recipient should match");
    assert_eq!(transfer.amount, 1000, "Amount should match");
    assert_eq!(transfer.status, TransferStatus::Pending, "Status should be pending");
}
