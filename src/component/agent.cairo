...
pub mod agent_component {
    ...
    > of IAgent<ComponentState<TContractState>> {
        ...
        fn get_agent(
            self: @ComponentState<TContractState>, agent_address: ContractAddress,
        ) -> Agent {
            assert(self.agent_exists.read(agent_address), TransferErrors::AGENT_NOT_FOUND);
            self.agents.read(agent_address)
        }
        fn get_agents_by_status(
            self: @ComponentState<TContractState>, status: AgentStatus, limit: u32, offset: u32,
        ) -> Array<Agent> {
        ...
        fn get_agents_by_region(
            self: @ComponentState<TContractState>, region: felt252, limit: u32, offset: u32,
        ) -> Array<Agent> {
            let mut agents = ArrayTrait::new();
        ...
        fn is_agent_authorized(
            self: @ComponentState<TContractState>, agent: ContractAddress, transfer_id: u256,
        ) -> bool {
        ...
        fn assign_agent_to_transfer(
            ref self: ComponentState<TContractState>, transfer_id: u256, agent: ContractAddress,
        ) -> bool {
            let caller = get_caller_address();
            let current_time = get_block_timestamp();
