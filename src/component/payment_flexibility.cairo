...
use starkremit_contract::base::types::{
    AutoPaymentSetup, EarlyPaymentInfo, PaymentConfig, PaymentFrequency, PaymentRecord,
    PaymentStatus, RoundData, RoundStatus,
};
...
pub mod payment_flexibility_component {
    ...
    pub struct Storage {
        ...
        payment_records: Map<
            (ContractAddress, u256), PaymentRecord,
        >, // (member, round_id) -> record
    ...
    pub enum Event {
        ...
        BulkAutoPaymentsProcessed: BulkAutoPaymentsProcessed,
    ...
    #[derive(Drop, starknet::Event)]
    pub struct BulkAutoPaymentsProcessed {
        processed_count: u32,
        timestamp: u64,
    }
    ...
    pub impl PaymentFlexibilityImpl<
    ...
    > of InternalTrait<TContractState> {
            ...
            self
                .emit(
                    Event::AutoPaymentSetup(
                        AutoPaymentSetupEvent {
                            member, token, amount, frequency, timestamp: get_block_timestamp(),
                        },
                    ),
                );
        }
        ...
        fn _process_early_payment(
            ...
            // Validate token is non-zero (retain actual token info)
            assert(token != 0.try_into().unwrap(), PaymentFlexibilityErrors::INVALID_TOKEN);
            ...
            assert(
                days_until_deadline >= config.min_early_payment_days,
                PaymentFlexibilityErrors::INVALID_AMOUNT,
            );
        ...
        fn _extend_grace_period(
            ref self: ComponentState<TContractState>, member: ContractAddress, extension_hours: u64,
        ) {
            self._assert_admin();
            ...
            assert(
                extension_hours <= config.max_grace_period_extension,
                PaymentFlexibilityErrors::INVALID_AMOUNT,
            );
                    ...
                    Event::GracePeriodExtended(
                        GracePeriodExtended {
                            ...
                            new_deadline: get_block_timestamp()
                                + (new_extension * SECONDS_PER_HOUR),
        ...
        fn _process_auto_payments(ref self: ComponentState<TContractState>) {
            ...
            if processed_count > 0_u32 {
                self
                    .emit(
                        Event::BulkAutoPaymentsProcessed(
                            BulkAutoPaymentsProcessed { processed_count, timestamp: current_time },
                        ),
                    );
            }
        }
            ...
            self
                .emit(
                    Event::SupportedTokenAdded(
                        SupportedTokenAdded { token, timestamp: get_block_timestamp() },
                    ),
                );
        }
        ...
        fn _remove_supported_token(
            ...
            // Find token index, swap with last element, and decrement count to keep array compact
            let count = self.supported_tokens_count.read();
            let mut i = 0;
            while i < count {
                let stored_token = self.supported_tokens.read(i);
                if stored_token == token {
                    let last_index = count - 1_u32;
                    if i != last_index {
                        let last_token = self.supported_tokens.read(last_index);
                        self.supported_tokens.write(i, last_token);
                    }
                    // Clear last slot and update count
                    self.supported_tokens.write(last_index, 0.try_into().unwrap());
                    self.supported_tokens_count.write(last_index);
                    break;
                }
                i += 1;
            }
            ...
            self
                .emit(
                    Event::SupportedTokenRemoved(
                        SupportedTokenRemoved { token, timestamp: get_block_timestamp() },
                    ),
                );
        }

        fn _update_payment_config(
            ref self: ComponentState<TContractState>, new_config: PaymentConfig,
        ) {
            self._assert_admin();
        ...
        fn _calculate_payment_status(
            ...
            if payment_record.amount == 0 {
                // No payment made
                let grace_period_end = round.deadline
                    + (config.grace_period_hours * SECONDS_PER_HOUR);
                ...
                if payment_record.is_early_payment {
                    ...
                    return PaymentStatus::PaidAfterGrace; // Payment made after grace period
                }
            }
        }

        fn _calculate_next_payment_date(
            self: @ComponentState<TContractState>, frequency: PaymentFrequency,
        ) -> u64 {
            let current_time = get_block_timestamp();
        ...
        fn _calculate_early_payment_discount(
        ...
        fn _execute_auto_payment(
            ...
            // Execute the actual payment transfer from the member to this contract
            let token_dispatcher = IERC20Dispatcher { contract_address: auto_setup.token };
            token_dispatcher.transfer_from(member, get_contract_address(), auto_setup.amount);
