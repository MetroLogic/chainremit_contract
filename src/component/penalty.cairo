...
use starkremit_contract::base::types::{
    DistributionData, MemberContribution, MemberPenaltyRecord, MemberShare, PenaltyConfig,
    PenaltyEventRecord, PenaltyEventType, RoundData, RoundStatus,
};
...
pub mod penalty_component {
    ...
    > of super::IPenalty<ComponentState<TContractState>> {
        ...
        fn calculate_distribution_data(self: @ComponentState<TContractState>) -> DistributionData {
                ...
                if contract_state.get_member_status(member_address) {
                    ...
                    if !penalty_record.is_banned {
                        ...
                        while round_id <= contract_state.get_round_ids() {
                            let contribution = contract_state
                                .get_member_contribution_data(round_id, member_address);
            ...
            // Compute final shares after total_compliant_contributions is known
            let mut computed_member_shares = ArrayTrait::new();
            let shares_span = member_shares.span();
            for member_share in shares_span {
                let mut computed_share: u256 = 0;
                if total_compliant_contributions > 0 {
                    computed_share = (*member_share.contribution * penalty_pool_amount)
                        / total_compliant_contributions;
                }
                if computed_share > 0 {
                    computed_member_shares
                        .append(
                            MemberShare {
                                member: *member_share.member,
                                share: computed_share,
                                contribution: *member_share.contribution,
                            },
                        );
                }
            }
            member_shares = computed_member_shares;

            let final_total_contributions: u256 = total_compliant_contributions;
            DistributionData {
                total_amount: penalty_pool_amount,
                member_shares,
                total_compliant_contributions: final_total_contributions,
            }
        }
    ...
    > of InternalTrait<TContractState> {
        ...
        fn apply_late_fee(
            ref self: ComponentState<TContractState>, member: ContractAddress, round_id: u256,
        ) {
            ...
            assert(
                get_block_timestamp() > round.deadline + total_grace_secs,
                PenaltyComponentErrors::NOT_LATE,
            );

            // If pool is disabled, do not collect late fees into pool
            assert(
                penalty_config.penalty_pool_enabled, PenaltyComponentErrors::PENALTY_POOL_DISABLED,
            );
        ...
        fn remove_strike(ref self: ComponentState<TContractState>, member: ContractAddress) {
            // Get current penalty record
            let mut penalty_record = self.member_penalties.read(member);
        ...
        fn ban_member(ref self: ComponentState<TContractState>, member: ContractAddress) {
            // Get current penalty record
            let mut penalty_record = self.member_penalties.read(member);
            penalty_record.is_banned = true;
        ...
        fn unban_member(ref self: ComponentState<TContractState>, member: ContractAddress) {
            // Get current penalty record
            let mut penalty_record = self.member_penalties.read(member);
            penalty_record.is_banned = false;
