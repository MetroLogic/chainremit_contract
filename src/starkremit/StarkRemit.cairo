...
pub mod StarkRemit {
    ...
    struct Storage {
        ...
        // RBAC & Multi-sig
        roles: Map<ContractAddress, u8>, // Role mapping (bitmask)
        pending_actions: Map<felt252, starkremit_contract::starkremit::access_control::rbac_multisig::PendingAction>,
        paused: bool, // Emergency pause flag
        pause_votes: Map<ContractAddress, bool>, // Community pause votes
        pause_vote_count: u8,
    }
    ...
    impl IStarkRemitImpl of IStarkRemit::IStarkRemit<ContractState> {
        ...
        /// Propose to add a new minter (multi-sig, RBAC, time delay)
        fn propose_add_minter(ref self: ContractState, minter_address: ContractAddress, delay_seconds: u64) -> felt252 {
            let params_hash = minter_address.into(); // For simplicity, just use address as hash
            access_control::propose_action(
                self.pending_actions,
                self.roles,
                access_control::PendingActionType::AddMinter,
                params_hash,
                delay_seconds
            )
        }

        /// Confirm a pending add minter action
        fn confirm_add_minter(ref self: ContractState, action_id: felt252) {
            access_control::confirm_action(self.pending_actions, self.roles, action_id);
        }

        /// Execute a pending add minter action after delay and confirmations
        fn execute_add_minter(ref self: ContractState, action_id: felt252, min_confirmations: u8) {
            let pending = self.pending_actions.read(action_id);
            assert(pending.action_type == access_control::PendingActionType::AddMinter, 'WrongActionType');
            access_control::execute_action(self.pending_actions, self.roles, action_id, min_confirmations);
            let minter_address = pending.params_hash.into();
            self.minters.write(minter_address, true);
            self.emit(MinterAdded { account: minter_address, added_by: pending.proposer });
        }

        /// Propose to remove a minter (multi-sig, RBAC, time delay)
        fn propose_remove_minter(ref self: ContractState, minter_address: ContractAddress, delay_seconds: u64) -> felt252 {
            let params_hash = minter_address.into();
            access_control::propose_action(
                self.pending_actions,
                self.roles,
                access_control::PendingActionType::RemoveMinter,
                params_hash,
                delay_seconds
            )
        }
        fn confirm_remove_minter(ref self: ContractState, action_id: felt252) {
            access_control::confirm_action(self.pending_actions, self.roles, action_id);
        }
        fn execute_remove_minter(ref self: ContractState, action_id: felt252, min_confirmations: u8) {
            let pending = self.pending_actions.read(action_id);
            assert(pending.action_type == access_control::PendingActionType::RemoveMinter, 'WrongActionType');
            access_control::execute_action(self.pending_actions, self.roles, action_id, min_confirmations);
            let minter_address = pending.params_hash.into();
            self.minters.write(minter_address, false);
            self.emit(MinterRemoved { account: minter_address, removed_by: pending.proposer });
        }

        /// Propose to mint tokens (multi-sig, RBAC, time delay)
        fn propose_mint(ref self: ContractState, to: ContractAddress, amount: u256, delay_seconds: u64) -> felt252 {
            let params_hash = to.into() + amount.low + amount.high;
            access_control::propose_action(
                self.pending_actions,
                self.roles,
                access_control::PendingActionType::Mint,
                params_hash,
                delay_seconds
            )
        }
        fn confirm_mint(ref self: ContractState, action_id: felt252) {
            access_control::confirm_action(self.pending_actions, self.roles, action_id);
        }
        fn execute_mint(ref self: ContractState, action_id: felt252, min_confirmations: u8) {
            let pending = self.pending_actions.read(action_id);
            assert(pending.action_type == access_control::PendingActionType::Mint, 'WrongActionType');
            access_control::execute_action(self.pending_actions, self.roles, action_id, min_confirmations);
            // Extract to/amount from params_hash if needed (store full params in production)
            // self.balances.write(to, self.balances.read(to) + amount);
            // self.emit(Minted { ... });
        }

        /// Propose to update user KYC level (multi-sig, RBAC, time delay)
        fn propose_update_kyc_level(ref self: ContractState, user: ContractAddress, new_level: KycLevel, delay_seconds: u64) -> felt252 {
            let params_hash = user.into() + new_level.into();
            access_control::propose_action(
                self.pending_actions,
                self.roles,
                access_control::PendingActionType::UpdateKYC,
                params_hash,
                delay_seconds
            )
        }
        fn confirm_update_kyc_level(ref self: ContractState, action_id: felt252) {
            access_control::confirm_action(self.pending_actions, self.roles, action_id);
        }
        fn execute_update_kyc_level(ref self: ContractState, action_id: felt252, min_confirmations: u8) {
            let pending = self.pending_actions.read(action_id);
            assert(pending.action_type == access_control::PendingActionType::UpdateKYC, 'WrongActionType');
            access_control::execute_action(self.pending_actions, self.roles, action_id, min_confirmations);
            // Extract user/new_level from params_hash if needed
            // ...update KYC logic...
        }

        /// Propose to deactivate a user (multi-sig, RBAC, time delay)
        fn propose_deactivate_user(ref self: ContractState, user: ContractAddress, delay_seconds: u64) -> felt252 {
            let params_hash = user.into();
            access_control::propose_action(
                self.pending_actions,
                self.roles,
                access_control::PendingActionType::Pause,
                params_hash,
                delay_seconds
            )
        }
        fn confirm_deactivate_user(ref self: ContractState, action_id: felt252) {
            access_control::confirm_action(self.pending_actions, self.roles, action_id);
        }
        fn execute_deactivate_user(ref self: ContractState, action_id: felt252, min_confirmations: u8) {
            let pending = self.pending_actions.read(action_id);
            assert(pending.action_type == access_control::PendingActionType::Pause, 'WrongActionType');
            access_control::execute_action(self.pending_actions, self.roles, action_id, min_confirmations);
            // ...deactivate user logic...
        }

        /// Propose to reactivate a user (multi-sig, RBAC, time delay)
        fn propose_reactivate_user(ref self: ContractState, user: ContractAddress, delay_seconds: u64) -> felt252 {
            let params_hash = user.into();
            access_control::propose_action(
                self.pending_actions,
                self.roles,
                access_control::PendingActionType::Unpause,
                params_hash,
                delay_seconds
            )
        }
        fn confirm_reactivate_user(ref self: ContractState, action_id: felt252) {
            access_control::confirm_action(self.pending_actions, self.roles, action_id);
        }
        fn execute_reactivate_user(ref self: ContractState, action_id: felt252, min_confirmations: u8) {
            let pending = self.pending_actions.read(action_id);
            assert(pending.action_type == access_control::PendingActionType::Unpause, 'WrongActionType');
            access_control::execute_action(self.pending_actions, self.roles, action_id, min_confirmations);
            // ...reactivate user logic...
        }

        /// Propose to suspend user KYC (multi-sig, RBAC, time delay)
        fn propose_suspend_user_kyc(ref self: ContractState, user: ContractAddress, delay_seconds: u64) -> felt252 {
            let params_hash = user.into();
            access_control::propose_action(
                self.pending_actions,
                self.roles,
        ...
        fn confirm_suspend_user_kyc(ref self: ContractState, action_id: felt252) {
            access_control::confirm_action(self.pending_actions, self.roles, action_id);
        }
        fn execute_suspend_user_kyc(ref self: ContractState, action_id: felt252, min_confirmations: u8) {
            let pending = self.pending_actions.read(action_id);
            assert(pending.action_type == access_control::PendingActionType::Pause, 'WrongActionType');
            access_control::execute_action(self.pending_actions, self.roles, action_id, min_confirmations);
            // ...suspend user KYC logic...
        }

        /// Propose to reinstate user KYC (multi-sig, RBAC, time delay)
        fn propose_reinstate_user_kyc(ref self: ContractState, user: ContractAddress, delay_seconds: u64) -> felt252 {
            let params_hash = user.into();
            access_control::propose_action(
                self.pending_actions,
                self.roles,
        ...
        fn confirm_reinstate_user_kyc(ref self: ContractState, action_id: felt252) {
            access_control::confirm_action(self.pending_actions, self.roles, action_id);
        }
        fn execute_reinstate_user_kyc(ref self: ContractState, action_id: felt252, min_confirmations: u8) {
            let pending = self.pending_actions.read(action_id);
            assert(pending.action_type == access_control::PendingActionType::Unpause, 'WrongActionType');
            access_control::execute_action(self.pending_actions, self.roles, action_id, min_confirmations);
            // ...reinstate user KYC logic...
        }

        /// Propose to set KYC enforcement (multi-sig, RBAC, time delay)
        fn propose_set_kyc_enforcement(ref self: ContractState, enabled: bool, delay_seconds: u64) -> felt252 {
            let params_hash = enabled.into();
            access_control::propose_action(
                self.pending_actions,
                self.roles,
                access_control::PendingActionType::UpdateKYC,
        ...
        fn confirm_set_kyc_enforcement(ref self: ContractState, action_id: felt252) {
            access_control::confirm_action(self.pending_actions, self.roles, action_id);
        }
        fn execute_set_kyc_enforcement(ref self: ContractState, action_id: felt252, min_confirmations: u8) {
            let pending = self.pending_actions.read(action_id);
            assert(pending.action_type == access_control::PendingActionType::UpdateKYC, 'WrongActionType');
            access_control::execute_action(self.pending_actions, self.roles, action_id, min_confirmations);
            // ...set KYC enforcement logic...
        }
    }
