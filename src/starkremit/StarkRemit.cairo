...
use starkremit_contract::base::errors::{
    EmergencyErrors, GovernanceErrors, GroupErrors, KYCErrors, RegistrationErrors, TransferErrors,
};
use starkremit_contract::base::events::*;
use starkremit_contract::base::types::{
    Agent, AgentStatus, AutoPaymentSetup, AutoScheduleConfig, ContributionAnalytics,
    ContributionRound, DistributionData, FinancialReport, GovRole, KYCLevel, KycLevel, KycStatus,
    LoanRequest, LoanStatus, MemberAnalytics, MemberContribution, MemberPenaltyRecord,
    MemberProfileData, MemberShare, ParameterBounds, ParameterHistory, PaymentConfig,
    PaymentFrequency, PenaltyConfig, PenaltyEventRecord, PenaltyEventType, RegistrationRequest,
    RegistrationStatus, RoundData, RoundPerformanceMetrics, RoundStatus, SavingsGroup,
    ScheduledRound, SystemHealthMetrics, TimelockChange, TransferData, TransferHistory,
    TransferStatus, UserKycData, UserProfile,
};
use starkremit_contract::component::analytics::{
    IAnalytics, IMainContractData as AnalyticsMainContractData,
};
use starkremit_contract::component::auto_schedule::{
    IAutoSchedule, IMainContractData as AutoScheduleMainContractData,
};
use starkremit_contract::component::emergency::IEmergency;
use starkremit_contract::component::member_profile::{
    IMainContractData as MemberProfileMainContractData, IMemberProfile,
};
use starkremit_contract::component::payment_flexibility::{
    IMainContractData as PaymentFlexibilityMainContractData, IPaymentFlexibility,
};
use starkremit_contract::component::penalty::{
    IMainContractData as PenaltyMainContractData, IPenalty,
};
...
pub mod StarkRemit {
    ...
    component!(
        path: payment_flexibility_component,
        storage: payment_flexibility,
        event: PaymentFlexibilityEvent,
    );
    ...
    impl PaymentFlexibilityInternalImpl =
        payment_flexibility_component::InternalImpl<ContractState>;

    // Analytics Component (exposed directly via component interface)
    ...
    // Member Profile Component (exposed directly via component interface)
    ...
    pub enum Event {
        ...
        AnalyticsEvent: analytics_component::Event,
        MemberProfileEvent: member_profile_component::Event,
        // System Management Events
    ...
    struct Storage {
        ...
        #[substorage(v0)]
        analytics: analytics_component::Storage,
        #[substorage(v0)]
        member_profile: member_profile_component::Storage,
    ...
    fn constructor(
        ...
        self.analytics.initializer(owner);
        // Initialize member profile component
    ...
    impl IStarkRemitImpl of IStarkRemit::IStarkRemit<ContractState> {
        // Wrapper: contribute_round -> updates analytics centrally
        fn contribute_round(ref self: ContractState, round_id: u256, amount: u256) {
            let caller = get_caller_address();
            let now = get_block_timestamp();

            // Read deadline to compute timeliness using real data
            let round_before = self.rounds.read(round_id);
            let is_on_time = now <= round_before.deadline;

            // Delegate to component for contribution state (no event emission)
            self.contribution_component.contribute_round(round_id, amount);

            // Centralized analytics update
            InternalFunctions::_update_analytics_after_contribution(
                ref self, caller, round_id, amount, is_on_time,
            );
        }
        // --- Penalty Functions ---
        ...
        fn remove_strike(ref self: ContractState, member: ContractAddress) {
            ...
            let current_record = self.penalty.get_member_penalty_record(member);
            let penalty_config = self.penalty.get_penalty_config();
            let will_be_unbanned = current_record.is_banned && current_record.strikes
                - 1 < penalty_config.max_strikes;
            ...
            if banned {
                ...
                self
                    .emit(
                        Event::MemberUnbanned(
                            MemberUnbanned {
                                member, unbanned_by: caller, timestamp: get_block_timestamp(),
                            },
                        ),
                    );
            }
        ...
        fn auto_complete_expired_rounds(
            ref self: ContractState, max_iterations: u32,
        ) -> (u32, bool) {
            self.ownable.assert_only_owner();
            self.auto_schedule._auto_complete_expired_rounds(max_iterations)
        }
        ...
        fn process_early_payment(
            ...
            self.ownable.assert_only_owner();
            self.payment_flexibility._process_early_payment(member, round_id, amount, token)
        }
        ...
        fn create_member_profile(ref self: ContractState, member: ContractAddress) {
            self.ownable.assert_only_owner();
            self.member_profile.create_member_profile(member);
        }
        ...
        fn update_reliability_rating(
            ...
            self.ownable.assert_only_owner();
            self.member_profile.update_reliability_rating(member, new_rating);
        }

        fn get_member_profile(self: @ContractState, member: ContractAddress) -> MemberProfileData {
            self.member_profile.get_member_profile(member)
        }

        fn add_to_waitlist(ref self: ContractState, member: ContractAddress) {
            self.ownable.assert_only_owner();
            self.member_profile.add_to_waitlist(member);
        }

        fn remove_from_waitlist(ref self: ContractState, member: ContractAddress) -> bool {
            self.ownable.assert_only_owner();
            self.member_profile.remove_from_waitlist(member)
        }

        fn get_waitlist_position(self: @ContractState, member: ContractAddress) -> u32 {
            self.member_profile.get_waitlist_position(member)
        }

        fn update_communication_preferences(
            ref self: ContractState, member: ContractAddress, preferences: felt252,
        ) {
            self.ownable.assert_only_owner();
            self.member_profile.update_communication_preferences(member, preferences);
        }

        fn send_member_message(ref self: ContractState, member: ContractAddress, message: felt252) {
            self.ownable.assert_only_owner();
            self.member_profile.send_member_message(member, message);
        }
    }
    ...
    // --- Emergency Functions ---
    #[generate_trait]
    pub impl Emergency of EmergencyTrait {
        ...
        fn emergency_complete_round(ref self: ContractState, round_id: u256) {
            ...
            if round.total_contributions > 0 {
                ...
                // Update round status and set completion timestamp
                round.status = RoundStatus::Completed;
                round.completed_at = get_block_timestamp();
        ...
        fn emergency_migrate_funds(ref self: ContractState, new_contract: ContractAddress) {
            ...
            assert(
                new_contract != starknet::get_contract_address(),
                EmergencyErrors::CANNOT_MIGRATE_TO_SELF,
            );
            ...
            assert(
                new_contract == registered_migration_target
                    || registered_migration_target.is_zero(),
    ...
    impl MainContractDataImpl of PenaltyMainContractData<ContractState> {
        fn get_member_contribution_data(
            ...
            MemberContribution {
                member, amount: contribution.amount, contributed_at: contribution.contributed_at,
            }
        }
    ...
    // Implementation of IMainContractData trait for payment flexibility component
    impl PaymentFlexibilityMainContractDataImpl of PaymentFlexibilityMainContractData<
        ContractState,
    > {
        fn get_round_data(self: @ContractState, round_id: u256) -> RoundData {
            let round = self.rounds.read(round_id);
    ...
    // Implementation of IMainContractData trait for analytics component
    impl AnalyticsMainContractDataImpl of AnalyticsMainContractData<ContractState> {
        fn get_round_data(self: @ContractState, round_id: u256) -> RoundData {
            let round = self.rounds.read(round_id);
            RoundData {
                deadline: round.deadline,
    ...
    // Implementation of IMainContractData trait for member profile component
    impl MemberProfileMainContractDataImpl of MemberProfileMainContractData<ContractState> {
        fn get_member_status(self: @ContractState, member: ContractAddress) -> bool {
            self.members.read(member)
        }
    ...
    impl InternalFunctions of InternalFunctionsTrait {
        ...
        fn update_round_analytics(ref self: ContractState, round_id: u256, status: RoundStatus) {
            ...
            let round = self.rounds.read(round_id);
            let participant_count = self.member_count.read(); // Simplified participant count
            self
                .analytics
                ._update_round_metrics(
                    round_id, status, round.total_contributions, participant_count,
                );
            self.analytics._update_contribution_analytics();
            self.analytics._update_system_health();
        }
        ...
        // Internal function to update analytics after contribution
        fn _update_analytics_after_contribution(
            ref self: ContractState,
            member: ContractAddress,
            round_id: u256,
            amount: u256,
            ...
            self.analytics._update_member_analytics(member, is_on_time, amount, round_id);

            // Update round performance metrics
            let round = self.rounds.read(round_id);
            let participant_count = 1; // Simplified - would need proper participant count logic
            self
                .analytics
                ._update_round_metrics(
                    round_id, round.status, round.total_contributions, participant_count,
                );
